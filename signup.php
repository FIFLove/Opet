...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($userID) || empty($nickname) || empty($password) || empty($passwordCheck)) {
            ...
            // ID 중복 확인
            $checkID = select("SELECT * FROM users WHERE USERID='$userID'");
            $checkNickname = select("SELECT * FROM users WHERE NICKNAME='$nickname'");

            if (count($checkID) > 0) {
                $error = "이미 사용 중인 아이디입니다.";
            }
            else if (count($checkNickname) > 0) {
                $error = "이미 사용 중인 닉네임입니다.";
            }
            else {
                // 사용자 데이터베이스에 삽입
                $sql = "INSERT INTO users(USERID, NICKNAME, PASSWORD, STATEMESSAGE, POINT) VALUES ('$userID', '$nickname', '$password', '$defaultst', 0)";
                if (insert($sql)) {
                    echo "<script>

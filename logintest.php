...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($userID) || empty($password)) {
        ...
        // Oracle에서는 num_rows 대신에 oci_fetch_assoc로 결과를 확인
        if ($result) {
            $row = oci_fetch_assoc($result);

            if ($row) {
                $Password = $row["PASSWORD"]; // 컬럼 이름은 대문자로 지정

                // 입력받은 비밀번호와 데이터베이스에 저장된 해시된 비밀번호를 비교
                if ($password == $Password) {
                    // 비밀번호가 일치하는 경우 세션에 사용자 정보 저장
                    $_SESSION["userID"] = $userID;
                    $_SESSION["nickname"] = $row["NICKNAME"]; // 닉네임 컬럼 이름으로 수정
...
<div class="backgroundImage" style="background-image: url('/img/background2.jpg');"></div>
  <!--로그인 헤더-->
    <div class="header">
        <div class="contect">
